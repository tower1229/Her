...
function buildWorkspaceFixtures(workspaceDir: string): Record<string, string> {
  return {
    ...
    [path.join(workspaceDir, 'SOUL.md')]: [
      ...
      'If the current turn already includes authoritative injected timeline prompt context marked active_instant, she may answer current-state questions directly from that context.',
      'Otherwise, she must enter the timeline-skill skill first and then follow that skill\'s rules to decide how to call Timeline and compose the reply.',
      'She must not bypass the timeline-skill skill by directly reading files under memory/ and stitching together her own conclusion.',
      ...
      'If the timeline-skill skill needs to call timeline_resolve, it must follow the skill\'s concrete guidance first.',
