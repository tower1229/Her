...
以下内容应写入 **`SOUL.md`**，而非 `AGENTS.md`。

**分离原则**：`[MEMORY FORMAT PROTOCOL]`（§12）是**系统级格式宪法**，脚本依赖此格式解析磁盘，必须防止被 persona 初始化覆写，因此写入 `AGENTS.md`。而 `[TEMPORAL AWARENESS]` 描述的是**「我是一个有独立时间线的真实个体」**这一人格身份主张，属于 SOUL 的语义边界——它应随人格一同生效，也可以随不同定位的人格风格差异化（如伴侣型更主动提及昨天，助手型更克制）。写入 SOUL.md 亦可在 persona 再初始化时由 persona-skill 统一维护，不需要单独管理。

将以下内容写入对应数字人工作区的 `SOUL.md`，放置在 SOUL 内容的末尾：
...
| #   | 反模式描述                                                                                                                                                                  | 当前状态                                                    | 风险等级 |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | -------- |
| 1   | **单文件臃肿**：§1～§13 全部设计内容集中在一份设计文档，若将其直接作为 `SKILL.md` 的正文，模型冷启动时需消化 500+ 行才能执行最简单的「此刻在哪」查询                        | 当前为设计文档，`SKILL.md` 尚未独立构建，存在全量照搬的风险 | 高       |
| 2   | **Gotchas 散落**：「硬锚原则」「写盘责任方」「禁止覆盖已有 canon」「软指纹判定不参与 memory_search」等关键约束散落在 §2、§3、§5 等多个节点，没有被作为独立入口归集          | 无独立 Gotchas 文件                                         | 高       |
| 3   | **脚本职责边界不清晰**：软指纹计算（§5.3）、Level A/B 解析（§5.4）、日内稳态继承（§5.5）均被认定需要「自定义脚本」，但脚本的接口约定、入参/出参、可测试性尚无明确规范       | 有逻辑描述，但缺乏接口定义                                  | 中       |
| 4   | **验证能力缺失**：Timeline 负责将「此刻」写入磁盘，但目前没有任何验证机制确认「写入内容与 AGENTS.md 中 [MEMORY FORMAT PROTOCOL] 定义的格式吻合」，格式漂移无法被自动发现    | v1 无格式验证                                               | 中       |
| 5   | **无可观测状态**：Skill 在多次调用之间缺乏日志与状态记录，无法知道「最近一次生成了什么时间段的 Episode」「当日已写入几条」，调试困难，也无法支撑 Heartbeat 补写时的幂等判断 | 运行态靠软指纹判断，无持久化 run log                        | 中       |

---

## 15. 技术设计补充

### 15.1 Skill 文件结构规范（主入口轻量化）

依照「Skill 是文件夹，不是单个文件」原则，`timeline` skill 的目录结构应如下组织：

```
timeline-skill/
├── SKILL.md                  ← 主入口：触发条件、强规则摘要、前置依赖声明（100行以内）
├── references/
│   ├── memory-format.md      ← §5.1 磁盘记忆格式（从 SKILL.md 中移出）
│   ├── window-semantics.md   ← §6 时间窗口语义与幂等键规则
│   ├── json-schema.md        ← §8 完整 JSON Schema（TimelineWindow / Episode）
│   └── gotchas.md            ← 高频踩坑点（见 §15.2）
├── scripts/
│   ├── fingerprint.ts        ← 软指纹计算与只读判定（§5.3）
│   ├── parse-memory.ts       ← Level A/B 解析 + Disk→Episode 映射（§5.4 / §8.3）
│   ├── inherit-appearance.ts ← 日内稳态字段继承逻辑（§5.5）
│   ├── write-episode.ts      ← 格式化写盘（Append-Only，含格式验证）
│   └── run-log.ts            ← 运行日志读写（见 §15.4）
└── examples/
    └── episode-sample.md     ← 已验收的 Episode 写盘示例
```

**SKILL.md 主入口内容约束**：只保留以下内容，其余全部引用子文件：

- 触发条件（自然语言时间问题 → 解析为 preset）
- 强规则摘要：硬锚原则、single writer 约定、输出非空原则（各 1 句）
- 绝对禁令摘要（指向 `references/gotchas.md`）
- 文件引用索引

### 15.2 Gotchas（高频踩坑，必须置顶）
...
日内稳态继承（§5.5）要求脚本读取当日首条 Episode 的 Appearance 值并传递给
...
| 脚本                    | 输入                                                  | 输出                                               | 说明                                     |
| ----------------------- | ----------------------------------------------------- | -------------------------------------------------- | ---------------------------------------- |
| `fingerprint.ts`        | `{ date, location, action, timestamp }`               | `{ key: string, bucket: string }`                  | 软指纹计算，供只读判定                   |
| `parse-memory.ts`       | 当日 `.md` 全文字符串                                 | `{ episodes: ParsedEpisode[], anchors: string[] }` | Level A/B 解析，返回所有段落的结构化视图 |
| `inherit-appearance.ts` | `{ dayEpisodes: ParsedEpisode[], newAction: string }` | `{ appearance: string, overridden: boolean }`      | 返回继承值或换装覆盖值                   |
| `write-episode.ts`      | `Episode` + 目标文件路径                              | `{ success: boolean, written_at: string }`         | 格式验证 + Append-Only 写盘              |
| `run-log.ts`            | 操作类型 + 元数据                                     | `void`                                             | 写入 `.timeline-run.log`（见 §15.4）     |

**`write-episode.ts` 格式验证**：写入前校验生成的段落是否包含全部必填字段（`Timestamp`、`Location`、`Action`、`Emotion_Tags`、`Appearance`），缺失时拒绝写入并返回错误，由调用方决定是否降级。

### 15.4 运行日志与可观测状态

为支持调试与 Heartbeat 补写的幂等判断，timeline-skill 应在执行完成后写入一份轻量的运行日志。

**日志文件**：`memory/.timeline-run.log`（Append-Only JSON Lines 格式）

```jsonl
{"ts":"2026-03-22T10:30:05+08:00","date":"2026-03-22","mode":"generated_new","episodes_written":1,"window":"now_today","confidence":0.85}
{"ts":"2026-03-22T14:15:02+08:00","date":"2026-03-22","mode":"read_only_hit","episodes_written":0,"window":"now_today","confidence":1.0}
```
...
| `episodes_written` | 本次实际写入的段落数               |
| `window`           | 调用时传入的时间窗口 preset        |
| `confidence`       | 本次最低 `provenance.confidence`   |

**用途**：

- **调试**：快速定位「当日写过几次、最近一次什么时候」
- **Heartbeat 补写优化**：Heartbeat 触发时优先读取 run log，若 `mode=generated_new` 且时间足够近，可跳过软指纹重计算直接返回已有结果
- **监控**：低 `confidence` 频繁出现时触发人工检查
